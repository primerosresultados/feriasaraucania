                                    ...
                                    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden shadow-sm">
                                        <div className="overflow-x-auto overflow-y-hidden">
                                            <table className="w-full border-collapse">
                                                <thead>
                                                    <tr style={{ backgroundColor: primaryColor }} className="text-white">
                                                        <th className="px-2 py-2.5 sm:px-3 sm:py-3 text-left font-bold text-[11px] sm:text-sm tracking-wide sticky left-0 z-10" style={{ backgroundColor: primaryColor }}>Categoría</th>
                                                        {recintoAuctions.map(([recinto, auction]) => {
                                                            ...
                                                            return (
                                                                <th key={recinto} className="px-2 py-2.5 sm:px-3 sm:py-3 text-center font-bold text-[11px] sm:text-xs border-l border-white/10">
                                                                    <div className="flex items-center justify-center gap-1 sm:gap-1.5">
                                                                        <div className="opacity-90">{recinto.charAt(0) + recinto.slice(1).toLowerCase()}</div>
                                                                        <button
                                                                            onClick={handleDownload}
                                                                            className="p-0.5 sm:p-1 rounded-md bg-white/20 hover:bg-white/30 transition-colors"
                                                                            title={`Descargar datos ${recinto.charAt(0) + recinto.slice(1).toLowerCase()}`}
                                                                        >
                                                                            <Download className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                                                        </button>
                                                                    </div>
                                                                    <div className="text-[9px] sm:text-[10px] mt-0.5 opacity-70 font-medium">{formatTableDate(auction.fecha)}</div>
                                                                </th>
                                                            );
                                                        })}
                                                    </tr>
